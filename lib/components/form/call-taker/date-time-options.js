...
class DateTimeOptions extends Component {
  ...
  _updateTimeInput = (time) => {
    console.log('???? _updateTimeInput', time)
  ...
  handleTimeChange = (evt) => {
    ...
    // check for valid shorthand entry (e.g. '4p', '801a') and auto-complete as standard time
    let shorthandEntry = null
    if (
      timeInput.charAt(timeInput.length - 1).toLowerCase() === 'a' ||
      timeInput.charAt(timeInput.length - 1).toLowerCase() === 'p'
    ) {
      if (!isNaN(timeInput.substring(0, timeInput.length - 1))) {
        const num = parseInt(timeInput.substring(0, timeInput.length - 1))
        if (num >= 1 && num <= 12) {
          shorthandEntry =
            num +
            ':00' +
            timeInput.charAt(timeInput.length - 1).toLowerCase() +
            'm'
        } else if (num >= 100) {
    ...
    if (shorthandEntry) {
      // if valid shorthand was entered, parse auto-completed entry
      const parsedTime = this.parseInputAsTime(shorthandEntry)
      ...
      // process current input as entered
    ...
    this.setState({ timeInput: shorthandEntry || timeInput })
  }
  ...
  render() {
    ...
    console.log('>>>>> render timeInput', timeInput)
